...
function retriggerInitOptionsPage(onComplete) {
  ...
  setTimeout(function() {
    initOptionsPage(onComplete);
  }, OPTIONS_INIT_RETRY_DELAY_MS);
}
...
function slideCard(newCardId) {
  ...
  if (currentCardIndex < newCardIndex) {
    ...
    }, CARD_SLIDE_DURATION_MS, function() {
      currentCard.hide();
    });
...
function setMonitorCountInfo(count, highlight) {
  ...
  elem.html(
    getMessage('optionsInfoMonitorsOnYourSystem', [count,
...
function submitBugReport() {
  ...
  data =
    '<style>* { font-family: Lucida Console; font-size: 12px; } .stack { font-size: 11px; color: #bbb; }</style>' + data;

  $.post('http://www.sidewise.info/submit_error/index.php', {
    'desc': desc,
    'data': data,
    'type': 'log'
...
function importState() {
  ...
  var importPrompt = $.prompt(html, {
    prefix: 'cleanblue',
    buttons: {
      'OK': true,
      'Cancel': false
    },
    callback: doImportState
  });
