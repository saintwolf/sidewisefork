...
function initRowTitleUpdater() {
  ...
  setInterval(function() {
    _update(0, HOUR_MS);
  }, REFORMAT_ALL_ROW_TITLES_1H_INTERVAL_MS);
  setInterval(function() {
    _update(HOUR_MS, DAY_MS);
  }, REFORMAT_ALL_ROW_TITLES_1D_INTERVAL_MS);
  setInterval(function() {
    _update(DAY_MS, null);
  }, REFORMAT_ALL_ROW_TITLES_OLDER_INTERVAL_MS);
}
  ...
  var rowTypes = {
    'page': {
      ...
      buttons: [{
        ...
        tooltip: getMessage('closed_pageRowButtonTip_close'),
...
function addPageTreeNodeToFancyTree(fancyTree, node, parentId, beforeSiblingId) {
  ...
  setTimeout(function() {
    fancyTree.updateRow.call(fancyTree, row, {
      icon: node.favicon
    });
  }, 10);
}
  ...
  switch (op) {
    ...
    case 'update':
      ...
      if (elem.status == 'complete' && row.attr('status') != 'complete' && row.attr('rowtype') == 'page' && row.attr('hibernated') == 'false') {
        ...
        setTimeout(function() {
          row.css('-webkit-transform', 'rotate(0deg)');
        }, 100);
        setTimeout(function() {
          row.css('-webkit-transform', '');
        }, 150);
      }
...
function onRowExpanderClick(evt) {
  bg.recentlyClosedTree.updateNode(evt.data.row.attr('id'), {
    collapsed: !(evt.data.expanded)
  });
}
  ...
  for (var i = 0; i < moves.length; i++) {
    ...
    $winMoveTests.each(function(i, e) {
      ...
      if ($row.attr('rowtype') == 'page' && $row.attr('hibernated') != 'true') {
        ...
        if ($toTopParent.attr('rowtype') == 'window' && !($toTopParent.is($oldTopParent))) {
          ...
          windowToWindowMoves[fromWindowId].push({
            node: node,
    ...
    if (needNewWindow) {
      ...
      chrome.windows.create(createDetails, function(win) {
        ...
        chrome.tabs.query({
          ...
          if (existingWinNode) {
            bg.tree.updateNode(existingWinNode, {
              id: 'X' + generateGuid()
            });
...
function moveTabToWindow(movingTabId, toWindowId, toPosition, afterFn) {
  ...
  chrome.tabs.move(movingTabId, {
    windowId: toWindowId,
    index: toPosition
  }, function() {
    chrome.tabs.update(movingTabId, {
      active: true
    }, function(tab) {
...
function allowDropHandler($fromRows, relation, $toRow) {
  ...
  if (movingNonPinnedTabs) {
    ...
    if ((relation == 'before' || relation == 'prepend') && $toRow.following('.ftRowNode[rowtype=page][hibernated=false][pinned=true]', $toRow.parentsUntil('.ftRoot > .ftChildren').last()).length > 0) {
      return false;
    }

    if (relation == 'prepend' && $toRow.is('[rowtype=window]')) {
      var toNode = bg.tree.getNode($toRow.attr('id'));
      if (toNode.following(function(e) {
  ...
  if (movingPinnedTabs) {
    ...
    if (toNode.topParent()) {
      ...
      if (fromTopParentIds.indexOf(toNode.topParent().id) >= 0) {
        ...
        if (toNode.preceding(function(e) {
            return e.isTab() && !e.pinned;
          }, toNode.topParent())) {
          return false;
        }
...
function onContextMenuShow($rows) {
  ...
  var $pages = $rows.filter(function(i, e) {
    return $(e).attr('rowtype') == 'page'
  });
  var $descendants = $rows.find('.ftRowNode');
  var $branches = $rows.add($descendants);
  var $branchesPages = $branches.filter(function(i, e) {
    return $(e).attr('rowtype') == 'page'
  });
  ...
  items.push({
    ...
    callback: onContextMenuItemRestoreRows
  });
  ...
  items.push({
    ...
    id: 'removeRow',
    icon: '/images/close.png',
    label: 'Permanently remove selected',
    callback: onContextMenuItemRemoveRows
  });
...
function onContextMenuItemCloseBranches($rows) {
  ...
  $rows.add($children).each(function(i, e) {
    removeRow($(e));
  });
}
...
function closeWindowRow(row) {
  ft.resetDragDropState(function() {
    ...
    if (threshold > 0 && childCount >= threshold) {
      var msg = getMessage('prompt_closeWindow', [childCount, (childCount == 1 ? getMessage('text_page') : getMessage('text_pages'))]);
      if (!confirm(msg)) {
        return;
      }
    }
...
function togglePageRowsHibernated($rows, hibernateAwakeState, activateAfterWaking) {
  var $hibernatedRows = $rows.filter(function(i, e) {
    return $(e).attr('hibernated') == 'true';
  });
  ...
  if (hibernateAwakeState >= 0 && $hibernatedRows.length > 0) {
    var ids = $hibernatedRows.map(function(i, e) {
      return $(e).attr('id');
    });
  ...
  var ids = $awakeRows.map(function(i, e) {
    return $(e).attr('id');
  });
...
function setPageRowPinnedState(row, pinned) {
  bg.tree.updateNode(row.attr('id'), {
    pinned: pinned
  });
  if (row.attr('hibernated') == 'true') {
    return;
  }
  chrome.tabs.update(getChromeId(row), {
    pinned: pinned
  });
}
