...
PageTree.prototype = {
  ...
  conformAllChromeTabIndexes: function(a) {
    if (a) {
      ...
      for (var a = this.tree.filter(function(a) {
        ...
        }), c = 0; c < a.length; c++) this.conformChromeTabIndexForNodeArray(a[c].children, !0, !0)
    } else {
      var b = this;
  ...
  dump: function() {
    ...
    return this.reduce(function(c, b, d) {
      ...
      return c + "\n" + padStringLeft(e, 3) + "/" + padStringLeft(b.index, 3) + "|" + padStringLeft(b.id, 30) + ": " + Array(-3 + 4 * (1 + d)).join(" ") + (b instanceof PageNode ? b.title : "window " + b.type + (b.incognito ? " incognito" : "")) + " @" + b.historylength + " R:" + b.referrer
    }, "")
  },
